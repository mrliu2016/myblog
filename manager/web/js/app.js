    ...
    function sidebarNav() {
        ...
        function activate(sidebar) {
            sidebar.find('a').each(function() {
                ...
                if (window_href.indexOf('?') >= 0) {
                    window_href = window_href.substring(0, window_href.indexOf('?'));
                }
                var new_href = href.split('/');
                var new_window = window_href.split('/');
                if (new_href.length >= 3 & new_href[1] == new_window[1] & new_href[2] == new_window[2]) {
                    var item = $(this).parents('li').addClass('active');
                    return false;
                }
                /*
                if (menu_href != "" && menu_href == address_href) {
                    var item = $(this).parents('li').addClass('active');
