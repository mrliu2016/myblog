...
//    }
    public function beforeAction($action)
    {
        ...
        if ($ticketFromCookie != null) {
            Yii::$app->controller->user = unserialize(base64_decode($ticketFromCookie));
            self::_refreshTicketExpire($ticketFromCookie);
        }
        return parent::beforeAction($action);
    }

    //没有设置7天免登录，则20分钟无操作自动退出（设置cookie过期时间为当前时间+20分钟）
    private function _refreshTicketExpire($ticketFromCookie)
    {
//        if (!Yii::$app->controller->user->rememberMe) {
            if (strpos($_SERVER['HTTP_HOST'], '3ttech.cn') === false) {
            ...
            $cookie = new Cookie([
                ...
                'value' => $ticketFromCookie,
                'expire' => time() + Constants::COOKIE_EXPIRE,
                'domain' => $domain
            ]);
