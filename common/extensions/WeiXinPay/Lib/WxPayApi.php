...
class WxPayApi
{
     ...
     */
    public static function unifiedOrder($url, $inputObj, $timeOut = 6)
    {
        //检测必填参数
        ...
        if (!$inputObj->isNotifyUrlSet()) {
            $inputObj->setNotifyUrl($inputObj->getNotifyUrl());//异步通知url
        }
        $inputObj->setAppId($inputObj->getAppId());//公众账号ID
        $inputObj->setMchId($inputObj->getMchId());//商户号
     ...
     * @throws WxPayException
     */
    public static function orderQuery($url, $inputObj, $timeOut = 6)
    {
        //检测必填参数
     ...
     * @return array
     */
    public static function transfers($url, $inputObj, $sslCert, $sslKey)
    {
        $inputObj->setAppId($inputObj->getAppId());//公众账号ID
        $inputObj->setMchId($inputObj->getMchId());//商户号
        $inputObj->setSpbillCreateIp($_SERVER['REMOTE_ADDR']);//终端ip
        ...
        $inputObj->setSign();
        $xml = $inputObj->ToXml();
        $response = self::postXmlCurl($xml, $url, true, 30, $sslCert, $sslKey);
...
//        }
     ...
     * @throws WxPayException
     */
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30, $sslCert = '', $sslKey = '')
    {
        $ch = curl_init();
        //设置超时
